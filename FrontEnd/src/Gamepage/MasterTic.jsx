  ...
  //handling each move
    ...
    const handlemove=(boardIndex,cellIndex)=>{
      ...
      if (tileClickRef.current) {
        ...
        tileClickRef.current.play().catch(() => {console.log("error playing sound")});
      }
...
// server side working functions
    ...
    const resetGame=()=>{
      setboards(Array(9).fill(null).map(()=>({cells:Array(9).fill(null),winner:null})))
      setcurrentplayer('X')
      setactiveBoard(null)
      setbigwinner(null)
      setstrikeline(null)
...
// useeffect to automate the reload when popstate occurs and also for preloading the video
  ...
  useEffect(()=>{
    ...
    socket.on('Gamestate',(gamestate)=>
      ...
      setplayersymbol(
        gamestate.players[0].id===socket.id?'X':
        gamestate.players[1].id===socket.id?'O':null
      )
...
useEffect(()=>{
  ...
  return ( 
    ...
    <div className="theme-switcher" onClick={()=>{setisdarkk(!isdark)}}>
    <i class="fa-solid fa-sun"></i>
    <i class="fa-solid fa-moon"></i>

    </div>
    <Whonext currentplayer={currentplayer} playersymbol={playersymbol} bigwinner={bigwinner} username={username} gameplayers={gameplayers} datatheme="light"/>
    {/* showing rematch after the win for both the players */}
    {bigwinner&&!rematcRequested&&!waitingforRematch &&(
      <button className='rematchbtn' onClick={handleRematchRequest}><span className='hideonmobile hideforlargetabs'>Rematch</span> <i class="fa-solid fa-rotate-right"></i></button>
    )}
